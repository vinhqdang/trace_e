...
We benchmark DEFER, wrapping the dominator-tree planner (default), against
AdvancedGreedy (AG), GreedyReplace (GR), the Sandwich lower bound (LSBM),
proximity-to-seeds, and the COMMIT ablation, on ca-GrQc, ca-HepTh, a
power-grid network, and a Barab\'asi--Albert graph ($n=2000$, $m=2$;
soc-Epinions1 did not finish within the compute budget available and is
left for future work), under constant activation probability and the
weighted-cascade (WC) model, seed sets of size 20 (random), and budget 20
...
Theorem~\ref{thm:isocut-separation} shows a worst-case family on which
...
seed-fixed test instances across six real contact and collaboration
networks, calibrated per network to the target infected fraction of
\citet{sterchi2026}. Table~\ref{tab:source-detection} reports top-1
accuracy (higher is better; top-5 accuracy and full per-method detail are
in \texttt{results/gnn\_benchmark\_sterchi/} in the code release).
