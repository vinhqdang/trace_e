...
$\Theta(\log n)$). The genuinely new technical content of TWIG relative to
their result is the treewidth generalization itself: a tree decomposition
admits frontier splits and merges that a path decomposition does not, and
handling those is exactly what Definition~\ref{def:config}'s
partition-with-pending-counts formalism and the linear-aggregation collapse
of Theorem~\ref{thm:twig-collapse} are built for. We do not know whether
their pathwidth algorithm already uses an analogous collapse to reach
linear time (their proceedings paper does not describe its internals at a
level we can compare against ours in detail), so we do not claim the
collapse idea itself is new to the harder treewidth setting, only that
handling splits and merges under it is; separately, the elimination-order
direction sensitivity of Proposition~\ref{prop:direction} has no analogue
on a path decomposition, where there is only one direction to speak of.
TWIG additionally supports blocking-set selection and greedy verification
via \texttt{greedy\_exact} and \texttt{brute\_force\_optimal\_block}
(Section~\ref{sec:twig-complexity}), but these are thin search loops around
an \texttt{evaluate} oracle and would wrap a pathwidth-restricted evaluator
just as readily, so we do not count them as a point of technical
distinction from \citet{nakamura2026}. Our treewidth dynamic program
follows the classical bounded-treewidth programming paradigm
\citep{bodlaender1988,cygan2015}.
...
well studied. \citet{golovinkrause2011} give the general framework of
adaptive submodularity under which many of these results are proved;
\citet{seemansinger2013} apply a two-stage adaptive-seeding idea
specifically to social-network influence maximization; and
\citet{tong2017}, already cited above for its RR-sketch-style approximation
guarantee, is itself framed as an explicitly \emph{adaptive} influence
maximization algorithm in dynamic networks, re-planning as the network
changes in a manner structurally similar to DEFER's round-by-round
re-planning, but for the maximization rather than the minimization
objective. For stochastic monotone submodular \emph{maximization}, the
adaptivity gap -- a term introduced by \citet{deangoemansvondrak2008} for
the stochastic knapsack problem -- is known to be \emph{bounded} by a
constant \citep{asadpournazerzadeh2016}; Theorem~\ref{thm:adaptivity-gap}
shows the opposite for IMIN, an \emph{unbounded} adaptivity gap, which we
attribute to $\FF(\cdot)$'s lack of supermodularity in $B$
\citep[already established by][]{xie2023} breaking the adaptive-submodularity
machinery that yields bounded gaps on the maximization side. To our
knowledge no prior work formalizes an adaptive variant of IMIN itself, or
proves a lemma of the shape of Lemma~\ref{lem:defer-free} (that deferred
commitment of a fixed one-shot plan is loss-free) or an
unbounded-adaptivity-gap result of the shape of
Theorem~\ref{thm:adaptivity-gap}, for this minimization problem.
...
$L\to\infty$ limit $2/(\Delta p)$. Taking $\Delta=4$, $p=0.01$, and
$L=5000$ (rather than $50$) gives one-shot spread $150.03$, DEFER spread
$3.02$, and a spread ratio of about $49.7$ -- close to the predicted
$2/(\Delta p)=50$ and already well past the saving-ratio limit of
$3.439$, which does not move as $p$ shrinks at fixed $\Delta$.
...
comparison (rather than reusing the samples $P'$ was optimized on) removes
the optimistic bias from optimizing and testing on the same data, and the
$1$-standard-error margin makes an accidental swap less likely than a bare
$\arg\max$ would; neither removes the residual chance that a truly
worse plan clears the margin on a given round's held-out draw. We do not
prove a bound on the resulting regret here, and Section~\ref{sec:experiments}
reports this residual honestly (Table~\ref{tab:defer-results}'s and
Table~\ref{tab:defer-allbudgets}'s footnoted cells are candidate instances
of it, not violations of the theorem, but also not proof that no bias
occurred).
\end{remark}
...
$B_{w+2}$ state count is exponential in treewidth regardless of the
...
Asadpour A, Nazerzadeh H (2016) Maximizing stochastic monotone submodular
  functions. \emph{Management Sci.} 62(8):2374--2391. [Verify exact page
  range prior to submission.]
...
Dean BC, Goemans MX, Vondr\'ak J (2008) Approximating the stochastic
  ...
  33(4):945--964. [Verify exact page range prior to submission.]
