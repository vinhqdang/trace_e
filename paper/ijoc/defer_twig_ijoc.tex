...
\item \textbf{DEFER} (Section~\ref{sec:defer}), an adaptive wrapper around
  ...
  and the adaptivity gap of IMIN is unbounded: the ratio of DEFER's saving
  over no intervention to the best one-shot's saving grows linearly in the
  maximum out-degree as the activation probability shrinks, and the ratio
  of the two policies' expected spreads is unbounded already at a fixed,
  minimal out-degree of two (Theorem~\ref{thm:adaptivity-gap}), with a
  matching, checkable construction.
...
network-reliability-flavored complexity analysis Theorem
\ref{thm:twig-correct} builds on; their own Exact-vs-GreedyReplace check
(their Table 3) uses a borrowed exact method on 100-node subgraphs at up
to four budget levels, smaller and less systematic than the
fully proof-verified, 60-instance study of Section~\ref{sec:twig-empirical}.
...
give a $(1-1/e-\delta)$-style guarantee via reverse-reachable sampling for
a related blocking-maximization objective; \citet{farajtabar2017} model
...
\citeauthor{zareie2021} survey this pre-2021 line. \citet{xiang2024} is a
...
graphs, a strictly more restrictive parameter than the treewidth TWIG uses
(every graph of pathwidth $k$ has treewidth at most $k$, but not
conversely -- e.g., a binary tree has treewidth $1$ and pathwidth
$\Theta(\log n)$). TWIG's genuinely new content relative to their result is
the treewidth generalization: a tree decomposition admits frontier splits
and merges a path decomposition does not, which is exactly what
Definition~\ref{def:config}'s partition-with-pending-counts formalism and
the collapse of Theorem~\ref{thm:twig-collapse} handle, and
Proposition~\ref{prop:direction}'s direction sensitivity has no
path-decomposition analogue. We do not know whether their algorithm
already uses an analogous collapse for the pathwidth case, so we claim
novelty only for handling splits and merges, not the collapse idea itself.
TWIG's \texttt{greedy\_exact}/\texttt{brute\_force\_optimal\_block}
(Section~\ref{sec:twig-complexity}) are thin search loops around an
\texttt{evaluate} oracle that would wrap a pathwidth-restricted evaluator
just as readily, so we do not count them as a point of distinction. Our
...
$N_1$, and blocks only the gateway (or first path vertex) of a branch it
has confirmed is live.}
\end{figure}
...
plan-protection test of Algorithm~\ref{alg:defer} only ever replaces the
leftover $L=P_{t-1}\setminus C_{t-1}$ by a new plan $P'$ whose held-out
sample-average saving exceeds $L$'s by more than one held-out standard
error, then the expected final spread of DEFER is at most that of the
one-shot solution $P_0$, up to the residual error of that one-sided test
(Remark~\ref{rem:winners-curse}).
\end{theorem}
...
For every $\Delta\ge 2$, $p\in(0,1)$, and path length $L\ge0$ there is an
instance with maximum out-degree $\Delta$, budget $1$, on which the best
one-shot solution and DEFER have expected \emph{savings} (spread reduction
relative to no intervention) $p(L+1)$ and $L\bigl(1-(1-p)^\Delta\bigr)$
respectively, so DEFER's saving is
$\frac{L}{L+1}\cdot\frac{1-(1-p)^\Delta}{p}$ times the one-shot saving -- a
factor increasing in $L$ to the limit $\bigl(1-(1-p)^\Delta\bigr)/p$, which
in turn tends to $\Delta$ as $p\to0$; hence this saving ratio is unbounded as
$\Delta\to\infty$. The ratio of the two policies' expected \emph{spreads}
themselves is a different, generally smaller quantity: it is at least
$\frac{(\Delta-1)p}{\Delta p-1+(1-p)^\Delta}$, which already diverges for
\emph{fixed} $\Delta\ge2$ as $p\to0$ (at rate $2/(\Delta p)$), so the spread
ratio, too, is unbounded, without needing $\Delta\to\infty$.
\end{theorem}
...
The frontier never exceeds size $2$ (Proposition~\ref{prop:direction}
applies to any path). Each orange configuration branches on its incident
live-edge outcome as described below; Table~\ref{tab:twig-example} lists
the resulting eight live-edge histories in full.}
\end{figure}
...
\node[ctr] (c) at (0,0) {$c\!=\!6$};
\foreach \k/\ang in {1/162,2/90,3/18,4/-54,5/-126} {
  \node[leaf] (l\k) at (\ang:1.5) {$\ell_\k\!=\!\k$};
  \draw (c) -- (l\k);
}
\draw[dashed, red!70!black, rounded corners] (-1.9,-1.9) rectangle (1.9,1.9);
\node[red!70!black, font=\scriptsize] at (0,-2.2) {frontier $=\{\ell_1,\dots,\ell_5\}$, size $5$, just before $c$'s step};
\end{scope}
...
realizations, but $B_{w+2}$ still grows very quickly in the treewidth $w$
...
$d^{out}_S$: out-degree of the seed set; $w$: treewidth; $B_{w+2}$: the
...
\begin{table}[h]
\TABLE
{Bell numbers $B_{w+2}$, illustrating why treewidth (not just $n$) bounds \texttt{ExactSpreadFast}\label{tab:bell-numbers}}
{\begin{tabular}{@{}lccccccc@{}}
\hline
\up $w$ & 1 & 2 & 3 & 4 & 5 & 8 & 10 \\
\hline
\down $B_{w+2}$ & 5 & 15 & 52 & 203 & 877 & 115{,}975 & 4{,}213{,}597 \\
\hline
\end{tabular}}
...
\item \textbf{COMMIT}: DEFER's loop with the push-down rule (step 4 of
  Algorithm~\ref{alg:defer}) replaced by committing the entire plan
  immediately every round; isolates \emph{when} a plan is committed from
  the effect of re-planning itself.
...
\hline
\end{tabular}}
{Bold marks DEFER when it is the best or tied-best adaptive/one-shot policy shown. Under the constant-probability model DEFER weakly improves on AG, its own planner, on every network, consistent with Theorem~\ref{thm:defer-dominates}; under the weighted-cascade model it mostly coincides with AG/GR (the push-down rule rarely fires when per-edge probabilities are the small, degree-normalized WC weights), and the one shortfall (ca-GrQc, WC) is a candidate instance of Remark~\ref{rem:winners-curse}'s residual bias risk on a 3-draw cell, not proof either way. COMMIT is at or below AG on 3 of 4 networks under the constant model and on all 4 under WC, illustrating that committing a full plan immediately, rather than deferring it, is the harmful ablation.}
\end{table}
...
holds in 10 of 12 cells; the two exceptions (ca-HepTh $b=50$, power-grid
$b=50$) are single-draw, small-margin reversals of the kind
Remark~\ref{rem:winners-curse} anticipates, not evidence against
Theorem~\ref{thm:defer-dominates}.
...
enough to measure precisely, consistent with a handful of re-planning
rounds each costing comparably to the single AG call. COMMIT, despite
...
locate the source of DEFER's advantage precisely: it is large when a
vertex's protection is cheap to buy \emph{after} it is known threatened
but expensive \emph{unconditionally}, i.e.\ when activation probabilities
are low relative to the graph's branching factor. The benchmark confirms
this rather than merely asserting it: DEFER's margin over its planner
shrinks as the budget grows (Figure~\ref{fig:defer-sweep}) and is small
under the weighted-cascade model, where per-edge probabilities are already
small and degree-normalized, so few vertices are individually
``expensive unconditionally, cheap conditionally'' enough for push-down to
defer them. A practitioner should ask whether the intervention has this
shape -- occasional high-fan-out gatekeepers, individually unlikely but
consequential -- rather than treat adaptivity as free and always better;
it is never worse (Theorem~\ref{thm:defer-dominates}), but the
``sometimes better'' can be negligible. DEFER also assumes full-adoption
feedback: the controller observes \emph{which} vertices activate each
round, not the underlying live-edge realization -- realistic for platform
moderation (who reposted, not which link) and epidemic containment (who
is infected, not who transmitted it), but ruling out partially-observed
activation; extending to noisy or delayed observation of $N_t$ is open.
...
$B_{w+2}$ state count is exponential in treewidth regardless of
$n$-independence, and Table~\ref{tab:twig-feasibility}'s real networks
already span tractable (karate, width 5) to infeasible by memory (dolphin,
width $\sim$10--11), two orders of magnitude below ca-GrQc. TWIG's role is
...
minimization -- DEFER (never worse than its wrapped planner, a
myopic-optimal commitment rule, an unbounded adaptivity gap) and TWIG (an
exact FPT dynamic program for IMIN's optimality gap, with a provable
elimination-order separation) -- both implemented and released.

% Endnotes (none currently used)
%\begingroup \parindent 0pt \parskip 0.0ex \def\enotesize{\normalsize} \theendnotes \endgroup

\ACKNOWLEDGMENT{To be added prior to submission.}
...
round, the plan-protection test only ever replaces the leftover
$L=P_{t-1}\setminus C_{t-1}$ by a new plan $P'$ whose held-out
sample-average saving exceeds $L$'s by more than one held-out standard
error, then the expected final spread of DEFER is at most that of the
one-shot solution $P_0$, up to the residual error of that one-sided test
(Remark~\ref{rem:winners-curse}).
...
plan that clears the plan-protection test for the remaining budget; the
committed prefix is identical. If every swap were only ever made when $P'$
is truly at least as good as $L$, induction over rounds would give the
claim exactly. The test instead accepts $P'$ whenever its held-out
sample-average saving exceeds $L$'s by more than one held-out standard
error -- a one-sided test against equally-good alternatives, using
concentration of the sample average (\citealp{xie2024}, applied per
...
$p\to0$) as $L\to\infty$; the ratio of the two policies' expected spreads is
at least $\frac{(\Delta-1)p}{\Delta p-1+(1-p)^\Delta}$, which diverges for
fixed $\Delta$ as $p\to0$.
\end{repeattheorem}
\begin{proof}{Proof}
...
is $\frac{(\Delta-1)p(L+1)}{(\Delta p-1+(1-p)^\Delta)L+\Delta p}$, whose
$L\to\infty$ limit is $\frac{(\Delta-1)p}{\Delta p-1+(1-p)^\Delta}$; for
fixed $\Delta$ as $p\to0$ the denominator behaves like
$\binom\Delta2p^2$ (the $O(p)$ terms cancel), so this limit behaves like
$\frac{(\Delta-1)p}{\binom\Delta2p^2}=\frac{2}{\Delta p}\to\infty$ --
already unbounded at $\Delta=2$, independently of the saving ratio's
$\Delta\to\infty$ argument. The two ratios are genuinely different
quantities -- the savings' ratio bound does not apply to the spreads'
ratio -- and both statements above are established directly from the same
exact closed forms, with no approximation beyond the stated $L\to\infty$
and $p\to0$ limits.\Halmos
\end{proof}
