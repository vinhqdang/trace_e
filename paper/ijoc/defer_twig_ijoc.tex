...
survey this pre-2021 line. \citet{xiang2024} is a recent
pertinence-constrained variant contemporaneous with \citet{xie2024}, the
...
\subsection{When is TWIG the right tool, and when is Monte Carlo unavoidable?}\label{sec:discussion-twig}
...
$n$-independence, and the real networks in Table~\ref{tab:twig-feasibility}
already span the practical range, from tractable (karate, width 5) to
infeasible by memory exhaustion (dolphin, width $\sim$10--11) two orders of
magnitude below ca-GrQc in size. TWIG's role is instead as a validation
instrument: on networks small enough for exact computation, it certifies
exactly how far greedy selection departs from optimal (Section
\ref{sec:twig-empirical}'s 60-instance study), which no Monte-Carlo
evaluation can certify -- sampling reduces variance but never eliminates
it, and greedy selection under noisy estimates is itself a source of bias
that more sampling does not remove. A practitioner wanting confidence that
a greedy IMIN implementation is correct, not merely fast, should validate
it against TWIG up to the treewidth frontier in
Table~\ref{tab:twig-feasibility} before trusting it at scale, analogous to
unit-testing a numerical solver against closed-form instances; three
implementation bugs caught only by exhaustive brute-force comparison
(Section~\ref{sec:twig-empirical}) show why that step is not optional.
...
remains to be measured. Second, TWIG's exact-validation scale limit is
discussed in Section~\ref{sec:discussion-twig} above; the 60-instance
optimality-gap study is a genuine, sampling-noise-free result on tiny
instances only. Third, the source-detection benchmark in
Section~\ref{sec:source-experiments} is an apples-to-apples reproduction of
a companion problem in the broader project this paper's code release
belongs to, not a contribution of this paper; we report it in full,
including its one incomplete network (\texttt{highschool2013}'s untrained
GNN cells), for transparency rather than omitting the gap.

\subsection{Concluding Remarks}\label{sec:conclusion}

We gave the first formal treatment of adaptivity for influence
minimization: DEFER, a wrapper provably never worse than the one-shot
planner it wraps, with a myopic-optimal commitment rule and a proof that
adapting helps by an unbounded rather than constant-factor amount; and
TWIG, an exact, fixed-parameter-tractable dynamic program for verifying
any IMIN heuristic's optimality gap without prior sampling confounds,
with a provable separation showing that tree-decomposition processing
direction is itself algorithmically consequential. Both are implemented,
tested against brute force, and released with the paper.
