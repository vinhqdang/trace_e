...
On the star-of-paths instance of Section~\ref{sec:defer-example} with
$\Delta=2$, $p=0.4$, $L=1$, $b=1$: a one-shot plan blocks one gateway
outright, giving expected spread $(\Delta-1)p(L+1)=1(0.4)(2)=0.8$. At
round $0$ the planner's single-vertex plan targets one gateway $a_i$, with
$q_{a_i}=p=0.4$, $c_{a_i}=1$ (its one child), and
$\lambda_0=p(L+1)=0.8$ (the plan's own marginal saving, since the plan
does not exhaust the budget of $1$ beyond this single pick); the push-down
condition $q_{a_i}(c_{a_i}+1/\lambda_0)=0.4(1+1.25)=0.9<1$ defers it. Let
$k\sim\mathrm{Binomial}(2,p)$ be the number of gateways that activate.
If $k\le1$, DEFER spends its unit (if any branch is active) exactly as the
worked example of Section~\ref{sec:defer-example} describes, sealing the
one active branch's continuation for the cost of the vertex already
reached: spread $\Delta p\cdot\mathbf 1[k=1]\cdot 1=k$ (each active
gateway contributes $1$; contributes no further path vertices). If $k=2$
(probability $p^2=0.16$), both gateways have activated but only one unit
of budget remains, so DEFER can block only one child; the other branch's
path vertex activates deterministically, contributing an extra $L=1$.
Expected spread is $\Pr[k=0]\cdot0+\Pr[k=1]\cdot1+\Pr[k=2]\cdot(2+L)
=0.36(0)+0.48(1)+0.16(3)=0.96$, exceeding the one-shot plan's $0.8$: full
DEFER, with push-down enabled, is strictly \emph{worse} than the planner
it wraps on this instance. The mechanism is budget contention exactly as
anticipated in the proof of Lemma~\ref{lem:pushdown}'s scope: deferring a
vertex is only free if the budget unit it releases is actually available
when needed, and here two simultaneously-deferred vertices compete for a
single unit.
\end{repeatremark}
...
harder, more concentrated seeding that activates a larger, more
overlapping set of branches immediately -- across the same networks and
budgets $10,20,50$ (Table~\ref{tab:defer-degree-seeds}). These numbers are
noisier (one draw per cell), and COMMIT is no longer uniformly dominated:
high-degree seeding compresses the rounds available for push-down to
exploit and gives COMMIT less opportunity to lock in a bad early
commitment. The qualitative ranking (DEFER weakly at or above AG) still
holds in 10 of 12 cells; the two exceptions where DEFER is strictly below
AG (ca-GrQc $b=10$, ca-HepTh $b=50$) are single-draw, small-margin
reversals of the kind Remark~\ref{rem:winners-curse} anticipates, not
evidence against Theorem~\ref{thm:defer-dominates}. (Power-grid $b=50$ is
a tie between AG and DEFER, not a reversal; on that and several other
cells COMMIT, not AG, is the single-draw winner, a separate effect of the
same high-degree seeding discussed above.)
...
of once. Table~\ref{tab:timing} confirms this directly: DEFER costs
roughly 2.5--7$\times$ AG's per-episode time, depending on the network
(2.5$\times$ on BA(2000) up to 6.9$\times$ on ca-GrQc), consistent with a
handful of re-planning rounds each costing comparably to the single AG
call. COMMIT, despite
executing the identical per-round algorithm as DEFER, costs \emph{more}
than DEFER on every network: its inferior plan quality
(Tables~\ref{tab:defer-results}, \ref{tab:defer-allbudgets}) leaves more
of the cascade active for longer, so it takes more rounds to die out, and
each round is an additional re-planning call -- a cost interaction
Theorem~\ref{thm:defer-complexity}'s per-round-times-rounds bound makes
explicit but a single-round bound would miss: a worse policy is also
slower, because it prolongs the process being controlled.
...
prior literature. Theorem~\ref{thm:isocut-separation} shows a worst-case
family on which frontier-isolation moves save $\Omega(N)$ where every
other rule saves $O(b)$. Table~\ref{tab:isocut} checks whether this
advantage is visible on the real SNAP-scale networks used throughout this
section: it is not. isocut is statistically indistinguishable from AG on
all four networks (it essentially never selects an isolation move over a
single-node pick), and defer\_cut is at best a marginal improvement over
plain DEFER (ca-HepTh) and a marginal loss elsewhere. This is not a
failure of the proof but a statement about which graphs the adversarial
construction's decoy-seed structure resembles: real collaboration and
infrastructure networks rarely contain a seed whose entire out-neighborhood
points into an otherwise-unreachable region, so the move earns its keep
only as a documented worst-case possibility, not a default-algorithm
component.
...
Table~\ref{tab:defer-nopush-parity} reports the fraction of spread saved by
full DEFER and by \texttt{DEFER-nopush} side by side, at budget 20, for
every (network, probability model) cell used in
Table~\ref{tab:defer-results}. This is a direct comparison of the two
policies on the same live-edge draws, so it does not depend on the
aggregation used to produce Table~\ref{tab:defer-results}'s
already-published numbers -- it only requires that \texttt{defer} and
\texttt{defer\_nopush} be compared to each other, which
\texttt{results/summary\_adaptive.csv} supports directly.
...
1--4, one to four seeds. For each instance,
...
live-edge subsets on graphs up to $n=9$ (2000+ test cases for
\texttt{ExactSpread}, 1000+ for \texttt{ExactSpreadFast}, not to be
confused with the DP's own \emph{configurations} of
Definition~\ref{def:config}), matching Theorems~\ref{thm:twig-correct}
and \ref{thm:twig-collapse}.
