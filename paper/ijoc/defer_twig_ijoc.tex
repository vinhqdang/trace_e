...
local conditions, though (as we show with a closed-form counterexample) it
does not inherit the same never-worse guarantee once several deferred
targets compete for the same budget; and the advantage of committing
gradually over committing all at once can be made arbitrarily large,
growing with the maximum number of people any single infected individual
can pass the outbreak to. We answer the second question with TWIG, a
method that computes the exact effect of any proposed blocking choice,
rather than estimating it by simulation, on networks small enough for
exact computation, and we use it to measure -- systematically and without
simulation noise clouding the measurement -- how often and how badly the
commonly used greedy blocking heuristic misses the truly best choice
(exactly optimal in 56 of 60 test instances, with a mean excess of $2.2\%$
and a worst case of $84.6\%$).
...
\citep{wang2024sandwich} -- is \emph{one-shot}: the entire budget is spent before the cascade is observed
...
\item \textbf{DEFER} (Section~\ref{sec:defer}), an adaptive wrapper around
  ...
  being replaced by a worse one, and (optionally) commits a planned vertex
  early only if a push-down inequality shows that waiting is not
  preferable. We prove: deferred commitment alone is free
  (Lemma~\ref{lem:defer-free}); the pure-deferral variant,
  \texttt{DEFER-nopush}, weakly dominates the one-shot planner it wraps
  (Theorem~\ref{thm:defer-dominates}); the push-down rule is the myopic
  optimum for the local, single-vertex commit-now/wait decision
  (Lemma~\ref{lem:pushdown}), but -- shown by a closed-form counterexample
  -- does not inherit this domination guarantee once several deferred
  targets can compete for the same budget
  (Remark~\ref{rem:pushdown-counterexample}); and the adaptivity gap of
  IMIN is unbounded: the ratio of DEFER's saving over no intervention to
  the best one-shot's saving grows linearly in the maximum out-degree as
  the activation probability shrinks, and the ratio of the two policies'
  expected spreads is unbounded already at a fixed, minimal out-degree of
  two (Theorem~\ref{thm:adaptivity-gap}), with a matching, checkable
  construction.
\item \textbf{TWIG} (Section~\ref{sec:twig}), an exact dynamic program for
  ...
  elimination order (Theorem~\ref{thm:twig-correct}) and, for a given
  order, exponential in the width $w$ it realizes -- a pathwidth-type
  parameter (Section~\ref{sec:treewidth-prelim}), not the graph's
  treewidth -- then reformulated via an exact linear-aggregation collapse
  (Theorem~\ref{thm:twig-collapse}) into a fixed-parameter tractable
  algorithm in $w$ rather than merely an XP one
  (Propositions~\ref{prop:xp-space}--\ref{prop:fpt-space}). We further show
  processing \emph{direction} is a first-class algorithmic choice
  (Proposition~\ref{prop:direction}, star-graph worst case), and that no
  elimination order keeps $w$ bounded on trees (complete binary trees),
  since TWIG's linear schedule has no join operation -- so TWIG is a
  bounded-pathwidth algorithm, not a bounded-treewidth one
...
\textbf{Rumor and misinformation blocking before IMIN.} The idea of
limiting an already-spreading process's damage by removing or countering
a small set of vertices predates IMIN's formal NP-hardness treatment by
over a decade. \citet{budak2011} counter-seed a rival campaign under
independent cascade; \citet{he2012} extend this to competitive linear
threshold; \citet{nguyen2012} and \citet{wang2013blocking} study node/edge
removal directly (essentially one-shot IMIN, without \citet{xie2023}'s
later hardness/inapproximability results); \citet{fan2013} add an
explicit removal-cost budget; \citet{wang2016drimux} and \citet{tong2018}
generalize to dynamic and multi-campaign environments (\citet{tong2017},
discussed below, is a maximization rather than blocking algorithm and is
not part of this line); \citet{farajtabar2017} model
the cascade as a Hawkes process steered by reinforcement learning;
\citet{yan2020} study blocker placement under a fixed budget;
...
None of this line considers the timing of commitment as a decision
variable in its own
...
\citet{golovinkrause2011} give the general framework of adaptive
submodularity under which many such results are proved;
\citet{seemansinger2013} apply a two-stage adaptive-seeding idea to
influence maximization; and \citet{tong2017}, cited above for its
RR-sketch guarantee, is itself an explicitly \emph{adaptive} maximization
algorithm that re-plans as the network changes, structurally similar to
DEFER but for the maximization objective. For stochastic monotone
submodular \emph{maximization}, the adaptivity gap -- a term introduced by
\citet{deangoemansvondrak2008} for the stochastic knapsack problem -- is
\emph{bounded} by a constant \citep{asadpournazerzadeh2016};
Theorem~\ref{thm:adaptivity-gap} shows the opposite for IMIN, an
\emph{unbounded} gap, attributable to $\FF(\cdot)$'s lack of
supermodularity in $B$ \citep[established by][]{xie2023} breaking the
machinery that bounds the maximization side. To our knowledge no prior
...
\end{tikzpicture}}
{DEFER's per-round control flow\label{fig:defer-flow}}
{Plan protection (top decision) chooses between the new plan and the
previous round's uncommitted leftover $L$ using a fresh held-out sample;
the push-down rule (bottom decision) then decides, for each planned vertex
exposed to the current frontier, whether to commit it now or defer it to a
child (Lemma~\ref{lem:pushdown}).}
\end{figure}
...
\end{tikzpicture}}
{The star-of-paths instance behind Theorem~\ref{thm:adaptivity-gap} and this worked example\label{fig:star-of-paths}}
{Each of the $\Delta=4$ gateways $a_i$ activates independently with
probability $p=0.1$ from seed $s$; an activated gateway deterministically
propagates down its own private length-$L$ path. A one-shot policy commits
before observing which gateways activate; DEFER waits one round, observes
$N_1$, and blocks the first path vertex of one branch it has confirmed is
live (the gateway itself can no longer be blocked once it has activated).}
\end{figure}
...
\end{tikzpicture}}
{Frontier evolution of \texttt{ExactSpread} on the path example\label{fig:twig-path-example}}
{Orange = currently in the frontier $F$; grey outline = already retired.
The frontier never exceeds size $2$: a path has treewidth $1$, and along
either endpoint-to-endpoint order the schedule only ever has the
just-retired vertex's successor and the newly-introduced vertex active at
once. Each orange configuration branches on its incident live-edge outcome
as described below; Table~\ref{tab:twig-example} lists the resulting eight
live-edge histories in full.}
\end{figure}
...
treewidth.
Figure~\ref{fig:star-direction} shows both directions on $K_{1,5}$.
...
\end{scope}
\end{tikzpicture}}
{Forward vs.\ reversed elimination order on $K_{1,5}$\label{fig:star-direction}}
{Numbers show each vertex's position in the elimination order. Left: with
the centre processed last, every leaf is introduced but cannot retire
until the centre arrives, so the frontier grows to all $5$ leaves. Right:
with the centre processed first, each leaf's only edge is processed the
instant it is introduced, so it retires immediately and the frontier never
exceeds $2$ vertices (Proposition~\ref{prop:direction}).}
\end{figure}
...
\item \textbf{defer\_cut}: DEFER wrapping \texttt{isocut} instead of plain
  AG, used only in Section~\ref{sec:isocut-practice} to check whether
  isocut's worst-case advantage survives once DEFER's adaptivity is
  layered on top.
\end{enumerate}
...
(Theorem~\ref{thm:defer-dominates}); full DEFER, with push-down enabled,
is not (Remark~\ref{rem:pushdown-counterexample}), though
...
-- push-down's theoretical gap from the proven guarantee does not show up
empirically here. DEFER also assumes full-adoption
feedback observed with negligible latency: the controller observes
\emph{which} vertices activate each round and can act before the next
round's propagation -- realistic for platform moderation (who reposted,
not which link) and epidemic containment (who is infected, not who
transmitted it) when suspensions or quarantines are fast relative to the
cascade's own timescale, but not otherwise; extending to noisy, delayed,
or partially-observed feedback is open.

\subsection{When is TWIG the right tool?}\label{sec:discussion-twig}

TWIG is not a substitute for the Monte-Carlo-based evaluation that AG,
GR, and DEFER rely on at SNAP scale
(Section~\ref{sec:defer-experiments}): Proposition~\ref{prop:fpt-space}'s
$B_{w+2}$ state count is exponential in the width $w$ regardless of
$n$-independence, and Table~\ref{tab:twig-feasibility}'s real networks
already span tractable (karate, $w=10$) to infeasible by memory (dolphin,
$w=19$), two orders of magnitude below ca-GrQc in $n$ alone. TWIG's role is
instead as a validation instrument: it certifies exactly how far greedy
departs from optimal (Section~\ref{sec:twig-empirical}'s 60-instance
study), which no Monte-Carlo evaluation can -- greedy under noisy
estimates is a source of bias more sampling does not remove. Validating
greedy IMIN against TWIG before trusting it at scale is analogous to
unit-testing a numerical solver against closed-form instances; three
implementation bugs caught only by exhaustive brute-force comparison show
why that step is not optional.

\subsection{Limitations and Concluding Remarks}\label{sec:conclusion}

Two limitations bound the scope honestly, beyond TWIG's scale limit
already discussed (Section~\ref{sec:discussion-twig}). First,
Section~\ref{sec:defer-experiments}'s benchmark covers four SNAP-scale
networks with few live-edge draws per cell, not a mature large-scale
evaluation, though the theory does not depend on this. Second,
Section~\ref{sec:source-experiments}'s benchmark reproduces a companion
project problem, not a contribution of this paper.

We gave the first formal treatment of adaptivity for influence
minimization -- DEFER (\texttt{DEFER-nopush} never worse than its wrapped
planner, a myopic-optimal but not globally guaranteed push-down
commitment rule, an unbounded adaptivity gap) and TWIG (an exact FPT
dynamic program for IMIN's optimality gap in the width $w$, with a
provable elimination-order separation) -- both implemented and released.

\textbf{Code and data availability.} DEFER, TWIG, and every baseline and
benchmark script used in this paper are released at
\url{https://github.com/vinhqdang/trace_e}. All networks used are either
generated by the released code (Barab\'asi--Albert, Erd\H{o}s--R\'enyi) or
the small public empirical datasets cited in
Section~\ref{sec:defer-experiments}; \texttt{results/summary\_adaptive.csv}
and the other \texttt{results/*.csv} files referenced throughout the paper
are the raw per-instance logs produced by the released benchmark scripts,
